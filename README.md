...
- `CHECK_INTERVAL` parameter is the time (in seconds) between each temperature check and potential profile change. A unit suffix is also accepted (`s`, `m`, `h` or `d`), so `90`, `90s` and `5m` are all valid. The container refuses to start on a value that isn't a strictly positive duration, as one the `sleep` command can't wait for would make the monitoring loop run at full speed instead. **Default** value is 60(s).
