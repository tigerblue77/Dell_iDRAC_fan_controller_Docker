...
- `DISABLE_THIRD_PARTY_PCIE_CARD_DELL_DEFAULT_COOLING_RESPONSE` parameter is a boolean that allows to disable third-party PCIe card Dell default cooling response. **Default** value is false.
  ...
  - The check runs once, only after the server has refused the IPMI command, and only in network mode : in local mode the container reaches the BMC through `/dev/ipmi0` and is given no iDRAC address or credentials, so there is nothing to address an HTTPS request to. It reads and never writes.
