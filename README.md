...
- `DISABLE_THIRD_PARTY_PCIE_CARD_DELL_DEFAULT_COOLING_RESPONSE` parameter is a boolean that allows to disable third-party PCIe card Dell default cooling response. **Default** value is false.
  ...
  - A write the iDRAC refuses is retried up to three times, one `CHECK_INTERVAL` apart, when the refusal describes a moment it was having — busy, its configuration job queue full, or a request that never completed. A refusal about the request or the credentials is concluded on the first answer instead, those not changing while the container runs. Either way the log says which, and how long it went on. This is not tunable, and does not need to be.
