...
- `LOW_INLET_TEMPERATURE_THRESHOLD` parameter is the intake air temperature below which the fan speed is reduced to `LOW_TEMPERATURE_FAN_SPEED`. **Default** value is (empty), the trigger being disabled. Negative values are accepted. When `HIGH_INLET_TEMPERATURE_THRESHOLD` is set too, this one must be lower than it: the two bound the same range from opposite ends.
