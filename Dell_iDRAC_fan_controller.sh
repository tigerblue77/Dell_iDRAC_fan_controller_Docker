...
# upgrading doesn't change the behavior of an existing configuration
...
if [ -n "$LOW_INLET_TEMPERATURE_THRESHOLD" ]; then
  ...
  if [ -n "$HIGH_INLET_TEMPERATURE_THRESHOLD" ] && [ "$LOW_INLET_TEMPERATURE_THRESHOLD" -ge "$HIGH_INLET_TEMPERATURE_THRESHOLD" ]; then
    print_error_and_exit "LOW_INLET_TEMPERATURE_THRESHOLD (${LOW_INLET_TEMPERATURE_THRESHOLD}°C) must be lower than HIGH_INLET_TEMPERATURE_THRESHOLD (${HIGH_INLET_TEMPERATURE_THRESHOLD}°C)"
  fi
fi
...
if [ -n "$LOW_CPU_TEMPERATURE_THRESHOLD" ]; then
  ...
  if [ "$LOW_CPU_TEMPERATURE_THRESHOLD" -ge "$CPU_TEMPERATURE_THRESHOLD" ]; then
    print_error_and_exit "LOW_CPU_TEMPERATURE_THRESHOLD (${LOW_CPU_TEMPERATURE_THRESHOLD}°C) must be lower than CPU_TEMPERATURE_THRESHOLD (${CPU_TEMPERATURE_THRESHOLD}°C)"
  fi
fi
...
if [ -n "$LOW_INLET_TEMPERATURE_THRESHOLD" ] || [ -n "$LOW_CPU_TEMPERATURE_THRESHOLD" ]; then
  if [ -z "$LOW_TEMPERATURE_FAN_SPEED" ]; then
    print_error_and_exit "LOW_TEMPERATURE_FAN_SPEED must be set when LOW_INLET_TEMPERATURE_THRESHOLD or LOW_CPU_TEMPERATURE_THRESHOLD is set"
  fi
  ...
  if [ "$DECIMAL_SPEED" -gt "$DECIMAL_FAN_SPEED" ]; then
    print_error_and_exit "LOW_TEMPERATURE_FAN_SPEED (${DECIMAL_SPEED}%) must not exceed FAN_SPEED (${DECIMAL_FAN_SPEED}%), the low temperature protection can only reduce the fan speed"
  fi
...
if [ -n "$HIGH_INLET_TEMPERATURE_THRESHOLD" ]; then
  echo "High inlet temperature threshold: ${HIGH_INLET_TEMPERATURE_THRESHOLD}°C (Dell default dynamic fan control profile applied above)"
fi
