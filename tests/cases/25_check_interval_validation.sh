...
function test_a_fractional_value_is_accepted() {
  # sleep waits for these correctly, so they are working configurations today and
  # refusing them would stop a container that never had a problem. The accepted set
  # is what sleep itself accepts rather than what the README documents : the point
  # of the check is to reject the values that don't wait, not the unusual spellings
  local VALUE
  for VALUE in "0.5" ".5" "1.5" "60.0" "0.5s" "2.5m"; do
    assert_check_interval_is_accepted "$VALUE"
  done
}

function test_a_fractional_zero_stops_the_controller() {
  # The zero check works on the digits rather than through arithmetic, which cannot
  # take a fractional value at all : "$((10#0.0))" is a syntax error, not a zero
  local VALUE
  for VALUE in "0.0" "0." ".0" "00.00" "0.0s"; do
    assert_check_interval_is_refused "$VALUE" "\"$VALUE\" is zero however it is spelled"
  done
}

function test_scientific_notation_stops_the_controller() {
  # sleep does wait 100 seconds for "1e2", so this is a deliberate narrowing :
  # supporting it would mean a hairier pattern and a zero check that has to cope
  # with "0e5", for a spelling nobody puts in an environment variable
  assert_check_interval_is_refused "1e2" "scientific notation is refused on purpose"
}
