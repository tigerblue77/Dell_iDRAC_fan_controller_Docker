...
function test_every_rule_that_was_argued_is_still_in_the_list() {
  ...
  # The two cases above are one-way : they walk what the file contains and refuse
  # anything outside the vetted set. Neither notices a rule that has GONE. An
  # editor conflict, a merge, or someone tidying the file can drop one and leave
  # the suite green while CLAUDE.md keeps promising a session that all three are
  # pre-approved -- so the session stops to ask for a command the document said it
  # would not have to, which reads as the grant having been refused rather than
  # lost. Completeness is the half that was missing
