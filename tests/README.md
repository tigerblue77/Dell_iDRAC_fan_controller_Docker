...
| `cases/72_fan_speed_interpolation.sh` | The ramp `HIGH_FAN_SPEED` turns on between `FAN_SPEED` and itself : the arithmetic, the hottest-CPU selection on a multi-socket server, startup validation, and that it is disabled by default and still falls back to Dell above `CPU_TEMPERATURE_THRESHOLD` |
